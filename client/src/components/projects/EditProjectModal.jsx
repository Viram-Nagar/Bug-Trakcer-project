...
function EditProjectModal({ isOpen, onClose, project }) {
  ...
  /*
    ✅ KEY FIX:
    Use LOCAL isLoading state instead of Redux selectProjectLoading.

    Problem was: selectProjectLoading becomes true when fetchProjects
    runs (on page load), and stays true during that fetch.
    If modal opens while projects are still loading → button
    is permanently disabled. ❌

    Solution: track only THIS form submission with local state. ✅
  */
  ...
  // Pre-fill form when project prop changes
  useEffect(() => {
    if (project) {
      setFormData({
        title: project.title || "",
          ...
          >
            <div
              className="bg-white rounded-2xl shadow-2xl w-full
                         max-w-md border border-gray-100
